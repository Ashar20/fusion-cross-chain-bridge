...
export class Official1inchEscrowIntegration {
  ...
  async discoverOfficialContracts() {
    ...
    try {
      ...
      if (this.contracts.escrowFactory === '0x0000000000000000000000000000000000000000') {
        ...
        this.contracts.escrowFactory = '0x084cE671a59bAeAfc10F21467B03dE0F4204E10C'
        this.contracts.resolver = '0x58A0D476778f6C84e945e8aD8e368A2B1491a6a8'
  ...
  async createOfficialEscrow(escrowParams) {
    ...
    try {
      ...
      // Estimate gas first
      const gasEstimate = await this.escrowFactory.createEscrow.estimateGas(
        token,
        amount,
        orderHash,
        deadline,
      ...
      const txResponse = await this.escrowFactory.createEscrow(
        ...
        {
          value: token === ethers.ZeroAddress ? amount : 0,
          gasLimit: gasEstimate * 2n // Use double the estimate for safety
        }
      )
