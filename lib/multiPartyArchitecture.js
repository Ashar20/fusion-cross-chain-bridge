...
export class MultiPartySwapArchitecture {
  constructor(ethProvider, ethSigner) {
    ...
    this.contracts = {
      htlcEscrow: '0x2C2e75cc8d8731234b1BA7bFd59C2417647c5CFF', // Real deployed contract
    ...
    // Contract ABI for the deployed HTLC contract
    this.contractABI = [
  ...
  async createMultiPartyEscrow(swapId, escrowParams) {
    ...
    try {
      ...
      // Call the actual contract function properly
      const txResponse = await this.htlcContract.createHTLCEscrow(
        ...
        hashlock,                   // _hashlock
        timelock,                   // _timelock
        token,                      // _token (0x0 for ETH)
        amount,                     // _amount
        srcChainId,                 // _srcChainId
        srcTxHash || 'pending_eos_tx', // _srcTxHash
        crossChainOrderId,          // _crossChainOrderId
        participants.ethRecipient,  // _beneficiary
  ...
  async executeProperWithdrawal(swapId, secret) {
    ...
    try {
      // Create withdrawal transaction that sends to proper recipient
      const withdrawalData = {
        type: 'MULTI_PARTY_WITHDRAWAL',
        swapId: swapId,
        secret: ethers.hexlify(secret),
        hashlock: hashlock,
        recipient: participants.ethRecipient, // Proper recipient, not initiator
        resolver: participants.resolver,
        resolverFee: participants.resolverFeeRate,
        timestamp: Date.now()
      }
      
      const withdrawalDataString = JSON.stringify(withdrawalData)
      const dataHex = '0x' + Buffer.from(withdrawalDataString).toString('hex')
      
      // Transaction to trigger withdrawal from escrow contract
      const withdrawalTx = {
        to: this.contracts.htlcEscrow, // Call escrow contract
        value: 0, // No value, just function call
        data: dataHex,
        gasLimit: 150000
      }
      
      console.log('\n💰 Executing withdrawal to proper recipient...')
      console.log(`Escrow Contract: ${withdrawalTx.to}`)
      console.log(`ETH Recipient: ${participants.ethRecipient}`)
      console.log(`Resolver: ${participants.resolver}`)
      console.log(`Resolver Fee: ${participants.resolverFeeRate / 100}%`)
      
      const txResponse = await this.ethSigner.sendTransaction(withdrawalTx)
      ...
      const withdrawalResult = {
        ...
        blockNumber: receipt.blockNumber,
        gasUsed: receipt.gasUsed.toString(),
        secretRevealed: ethers.hexlify(secret),
      ...
      console.log('✅ Multi-party withdrawal completed!')
      console.log(`📦 Block: ${receipt.blockNumber}`)
      console.log(`⛽ Gas Used: ${receipt.gasUsed}`)
      console.log('🎉 Funds sent to proper recipient (not self)!')
