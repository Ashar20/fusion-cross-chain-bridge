...
export class Official1inchFusionIntegration {
  ...
  async submitToOfficialResolvers(orderData) {
    ...
    const submissionData = {
      ...
      order: this.serializeBigInts(order),
  ...
  async signOrder(order) {
    ...
    // Mock signature for testing - in production this would be real EIP-712 signature
    const mockSignature = '0x' + 'a'.repeat(130) // 65 bytes = 130 hex chars
