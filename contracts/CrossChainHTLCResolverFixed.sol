...
/**
 * @title CrossChainHTLCResolverFixed
 * @dev FIXED Cross-chain HTLC resolver for Ethereum ↔ Algorand atomic swaps
 * 
 * 🔧 FIXES APPLIED:
 * - DEFAULT_TIMELOCK changed from 24 hours to 1 hour (3600 seconds)
 * - Removed incorrect timelock check from executeCrossChainSwap
 * - Proper HTLC claiming logic (can claim anytime with secret)
 * 
...
contract CrossChainHTLCResolverFixed is ITakerInteraction {
    error OnlyOwner();
    error NotTaker();
    error OnlyLOP();
    ...
    // 🎯 CROSS-CHAIN CONFIGURATION - FIXED!
    uint256 public constant ALGORAND_CHAIN_ID = 416002; // Testnet
    uint256 public constant DEFAULT_TIMELOCK = 1 hours; // FIXED: 1 hour for development (was 24 hours)
    ...
    /**
     * @dev Execute cross-chain swap with secret reveal - FIXED!
     * @param _orderHash Order hash to execute
     * @param _secret Secret that matches the hashlock
     * 
     * 🔧 FIX: Removed incorrect timelock check - claiming can happen anytime with secret
     */
    function executeCrossChainSwap(
        ...
        // 🔧 FIXED: Removed this line - require(block.timestamp < order.timelock, "Order expired");
    ...
    function takerInteraction(
        ...
        // Decode the extra data to get the order hash and secret
        (bytes32 orderHash, bytes32 secret) = abi.decode(extraData, (bytes32, bytes32));
        
        // Execute the cross-chain swap
        executeCrossChainSwap(orderHash, secret);
    }
} 