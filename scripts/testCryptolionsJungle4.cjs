...
/**
 * 🌐 Test Cryptolions Jungle4 API Endpoint
 * 
 * This script tests the Cryptolions Jungle4 endpoint to see if it resolves
 * the indexer/query issues we've been experiencing.
 */

const { JsonRpc } = require('eosjs');

class CryptolionsJungle4Tester {
    constructor() {
    ...
    async testCryptolionsEndpoint() {
        console.log('🌐 Testing Cryptolions Jungle4 API Endpoint');
    ...
    displayResults(results) {
        console.log('\n📊 Cryptolions Jungle4 Test Results');
        console.log('=' .repeat(60));
        ...
        console.log('\n🎯 Recommendation:');
        if (score >= 4) {
            console.log(`   🏆 EXCELLENT! Cryptolions endpoint works perfectly!`);
            console.log(`   📋 Switch to this endpoint for full functionality.`);
            console.log(`   🌐 Update your scripts to use: ${this.endpoint}`);
        } else if (score >= 3) {
            console.log(`   🟡 GOOD! Cryptolions endpoint has some issues but is usable.`);
            console.log(`   📋 Consider switching to this endpoint.`);
        } else {
            console.log(`   🔴 POOR! Cryptolions endpoint has the same issues as others.`);
            console.log(`   💡 This confirms it's a Jungle4 network-wide problem.`);
        }
    }
}

// Run the Cryptolions tester
async function main() {
    const tester = new CryptolionsJungle4Tester();
    
    try {
        await tester.testCryptolionsEndpoint();
    } catch (error) {
        console.error('❌ Cryptolions testing failed:', error.message);
        process.exit(1);
    }
}
...
module.exports = { CryptolionsJungle4Tester }; 