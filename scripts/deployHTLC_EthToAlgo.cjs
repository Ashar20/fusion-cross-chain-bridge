#!/usr/bin/env node
/**
 * Deploy ETH→ALGO minimal HTLC contract to Algorand Testnet
 */
...
const contractPath = 'contracts/algorand/AlgorandHTLCBridge_EthToAlgo.py';

function compileTeal() {
    const teal = execSync(`source pyteal_env/bin/activate && python3 ${contractPath}`, { encoding: 'utf8', shell: '/bin/bash' });
    fs.writeFileSync('contracts/algorand/AlgorandHTLCBridge_EthToAlgo.teal', teal);
    return teal;
}
...
async function deploy() {
    ...
    console.log('✅ Deployed ETH→ALGO HTLC App ID:', appId);
    fs.writeFileSync('ALGORAND_HTLC_ETH_TO_ALGO_DEPLOYMENT.json', JSON.stringify({ appId, txId }, null, 2));
}
