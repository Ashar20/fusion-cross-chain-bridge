...
/**
 * 🎯 COMPLETE ETH → ALGO ATOMIC SWAP
 * 
 * Continue the swap from where we left off with the extracted order hash
...
class CompleteETHtoALGOSwap {
    constructor() {
        console.log('🎯 COMPLETING ETH → ALGO ATOMIC SWAP');
        console.log('=====================================');
        console.log('✅ Using extracted order hash from successful transaction');
    ...
    async initialize() {
        ...
        // 🔧 FIXED: Use the same secret and hashlock from the successful transaction
        // From the extracted transaction: hashlock = 0x04496c310a28bdca0e86a43cf816b16ed744811c54bb0ef735e6007c9190e924
        // We need to find the secret that generates this hashlock
        this.swapParams = {
            secret: crypto.randomBytes(32), // We'll need to find the correct secret
            hashlock: '0x04496c310a28bdca0e86a43cf816b16ed744811c54bb0ef735e6007c9190e924',
            timelock: 1754184072 // From the extracted transaction
        };
        ...
        console.log('🔒 SWAP PARAMETERS (FROM EXTRACTED TX):');
    ...
    async step5_executeCrossChainSwap() {
        ...
        try {
            // ⚠️ IMPORTANT: We need the correct secret that matches the hashlock
            // For now, let's try with a random secret to see the error
            ...
            const tx = await this.resolver.executeCrossChainSwap(this.orderHash, testSecret);
            ...
            console.log('');
            console.log('💡 This is expected - we need the correct secret that matches the hashlock');
            console.log('💡 The hashlock from the transaction is: 0x04496c310a28bdca0e86a43cf816b16ed744811c54bb0ef735e6007c9190e924');
            console.log('💡 We need to find the secret that generates this hashlock');
            console.log('');
            throw error;
        }
    }
    ...
    async generateProof() {
        console.log('📜 GENERATING PARTIAL SWAP PROOF');
        console.log('=================================');
        
        try {
            const proof = {
                testType: 'ETH_TO_ALGO_PARTIAL_SWAP_PROOF',
                timestamp: new Date().toISOString(),
                success: false,
                reason: 'Secret not found for hashlock',
                swapDetails: {
                    direction: 'ETH → ALGO',
                ...
                transactions: {
                    ...
                    algorandHTLC: this.algoHTLCTxId || 'Not created'
                },
                addresses: {
                ...
                cryptographicProof: {
                    hashlockFromTx: this.swapParams.hashlock,
                    secretStatus: 'Not found - need to brute force or find original secret',
                    verification: 'Pending - requires correct secret'
                },
                networks: {
                ...
                status: 'PARTIAL_SUCCESS - Ethereum order created, Algorand HTLC created, but secret not found for claiming'
            };
            
            // Save proof to file
            const proofPath = 'ETH_TO_ALGO_PARTIAL_SWAP_PROOF.json';
            fs.writeFileSync(proofPath, JSON.stringify(proof, null, 2));
            
            console.log('✅ Partial swap proof generated successfully!');
    ...
    async runCompleteTest() {
        try {
            console.log('🚀 CONTINUING ETH → ALGO ATOMIC SWAP');
            console.log('=====================================');
            console.log('');
            ...
            // Step 5: Execute cross-chain swap (will fail due to wrong secret)
            try {
                await this.step5_executeCrossChainSwap();
            } catch (error) {
                console.log('⚠️ Expected failure - secret not found');
            }
            
            // Generate proof
            await this.generateProof();
            
            console.log('🎉 PARTIAL TEST COMPLETED!');
            console.log('==========================');
            console.log('✅ Ethereum order created successfully');
            console.log('✅ Algorand HTLC created successfully');
            console.log('❌ Cross-chain swap failed - secret not found');
            console.log('💡 Need to find the correct secret for hashlock: 0x04496c310a28bdca0e86a43cf816b16ed744811c54bb0ef735e6007c9190e924');
            console.log('');
...
async function main() {
    try {
        const test = new CompleteETHtoALGOSwap();
