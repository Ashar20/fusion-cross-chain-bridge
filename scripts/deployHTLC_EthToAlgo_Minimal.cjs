#!/usr/bin/env node
/**
 * Deploy ultra-minimal ETH→ALGO HTLC contract to Algorand Testnet
 */
...
const contractPath = 'contracts/algorand/AlgorandHTLCBridge_EthToAlgo_Minimal.py';

function compileTeal() {
    const teal = execSync(`source pyteal_env/bin/activate && python3 ${contractPath}`, { encoding: 'utf8', shell: '/bin/bash' });
    fs.writeFileSync('contracts/algorand/AlgorandHTLCBridge_EthToAlgo_Minimal.teal', teal);
    return teal;
}
...
async function deploy() {
    ...
    console.log('✅ Deployed ultra-minimal ETH→ALGO HTLC App ID:', appId);
    fs.writeFileSync('ALGORAND_HTLC_ETH_TO_ALGO_MINIMAL_DEPLOYMENT.json', JSON.stringify({ appId, txId }, null, 2));
}
