...
/**
 * 🎯 CREATE ALGO TO ETH LOP ORDER
 * 
 * Creates a real ALGO to ETH limit order for the LOP system
...
async function createAlgoToEthLOPOrder() {
    console.log('🎯 CREATING ALGO TO ETH LOP ORDER');
    console.log('==================================\n');
    ...
    try {
        ...
        const intent = {
            ...
            makerAmount: ethers.parseEther('1'), // 1 ALGO
            takerAmount: ethers.parseEther('0.001'), // 0.001 ETH
        ...
        // Submit limit order
        // Note: For ALGO to ETH, we don't need to send ETH value since user is selling ALGO
        const tx = await contract.submitLimitOrder(
            ...
            { 
                gasLimit: 500000
                // No value needed since user is selling ALGO, not ETH
            }
        );
        ...
        if (orderId) {
            ...
            const orderDetails = {
                ...
                description: 'User selling 1 ALGO for 0.001 ETH'
            };
            
            fs.writeFileSync('algo-to-eth-lop-order.json', JSON.stringify(orderDetails, null, 2));
            console.log('📄 Order details saved to algo-to-eth-lop-order.json');
        ...
        if (error.message.includes('insufficient funds')) {
            console.log('\n💡 TROUBLESHOOTING:');
            console.log('   - Check wallet ETH balance for gas fees');
...
if (require.main === module) {
    createAlgoToEthLOPOrder();
}

module.exports = { createAlgoToEthLOPOrder }; 