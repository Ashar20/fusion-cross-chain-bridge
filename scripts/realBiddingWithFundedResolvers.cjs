...
/**
 * 🚀 REAL BIDDING WITH FUNDED RESOLVERS
 * 
 * Uses the newly generated resolver wallets with private keys
...
class RealBiddingWithFundedResolvers {
    constructor() {
        this.contract = null;
    ...
    async initialize() {
        console.log('🚀 REAL BIDDING WITH FUNDED RESOLVERS');
        console.log('=====================================\n');
        ...
        try {
            ...
            // Load funded resolvers
            await this.loadFundedResolvers();
    ...
    async loadFundedResolvers() {
        console.log('\n🔧 Loading funded resolvers from .env.resolvers...');
        ...
        const resolverConfigs = [
            {
                ...
                privateKey: process.env.RESOLVER_1_PRIVATE_KEY,
            ...
            },
            {
                ...
                privateKey: process.env.RESOLVER_2_PRIVATE_KEY,
            ...
            },
            {
                ...
                privateKey: process.env.RESOLVER_3_PRIVATE_KEY,
            ...
            },
            {
                ...
                privateKey: process.env.RESOLVER_4_PRIVATE_KEY,
        ...
        for (const config of resolverConfigs) {
            ...
            // Create resolver wallet with private key
            const resolverWallet = new ethers.Wallet(config.privateKey, this.provider);
    ...
    async placeCompetitiveBids(orderId, intent) {
        ...
        for (const resolver of this.resolvers) {
            ...
            try {
                // Place bid using resolver's wallet
                const tx = await this.contract.connect(resolver.wallet).placeBid(
                    orderId,
                    intent.makerAmount,
                    improvedAmount,
                    gasEstimate
    ...
    async executeBestBid(orderId, bids, secret) {
        ...
        try {
            ...
            const executeTx = await this.contract.connect(bestBid.resolver.wallet).selectBestBidAndExecute(
                orderId,
                0, // First bid index
                secret
            );
    ...
    async run() {
        try {
            ...
            console.log('\n🎉 REAL BIDDING WITH FUNDED RESOLVERS COMPLETE!');
            console.log('=================================================');
            console.log('✅ Order created and submitted');
            console.log('✅ Competitive bids placed using funded resolvers');
            console.log('✅ Best bid executed');
            console.log('✅ Complete LOP intent workflow demonstrated!');
...
if (require.main === module) {
    const demo = new RealBiddingWithFundedResolvers();
    demo.run().catch(console.error);
}

module.exports = RealBiddingWithFundedResolvers;
